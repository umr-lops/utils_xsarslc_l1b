...
def compute_cwave_parameters(xs, save_kernel=False, kmax=2*np.pi/25, kmin=2*np.pi/600, Nk=4, Nphi=5):
    ...
    cwave_parameters = ((kernel.cwave_kernel*xxsmn)*dky*dky).sum(dim=['k_rg','k_az']).rename('cwave_params').to_dataset()
...
def compute_kernel(krg, kaz, save_kernel=False, kmax=2*np.pi/25, kmin=2*np.pi/600, Nk=4, Nphi=5):
    ...
    Gnk = xr.combine_by_coords([gegenbauer_polynoms(alpha_k,ik-1,lbda=3/2.)*coef(ik-1)*nu(alpha_k).assign_coords({'k_gp':ik}).expand_dims('k_gp') for ik in np.arange(Nk)+1])
    Fnphi = xr.combine_by_coords([harmonic_functions(alpha_phi,iphi).assign_coords({'phi_hf':iphi}).expand_dims('phi_hf') for iphi in np.arange(Nphi)+1]) 
...
def gegenbauer_polynoms(x,nk,lbda=3/2.):
    ...
    Cnk = (1/nk) * ( 2*x*(nk+lbda-1)*gegenbauer_polynoms(x,nk-1,lbda=lbda) - (nk+2*lbda-2)*gegenbauer_polynoms(x,nk-2,lbda=lbda) )
    Cnk = (1/nk) * ( 2*x*(nk+lbda-1)*gegenbauer_polynoms(x,nk-1,lbda=lbda) - (nk+2*lbda-2)*gegenbauer_polynoms(x,nk-2,lbda=lbda) )
