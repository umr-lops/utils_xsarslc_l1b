...
def symmetrize_xspectrum(xs, dim_range="k_rg", dim_azimuth="k_az"):
    ...
    if not xs.sizes["k_az"] % 2:
        xs = xr.concat(
            [xs, xs[{"k_az": 0}].assign_coords({"k_az": -xs.k_az[0].data})], dim="k_az"
        )

    mirror = np.conj(xs[{dim_range: slice(None, 0, -1)}])
    mirror = mirror.assign_coords(
        {dim_range: -mirror[dim_range], dim_azimuth: -mirror[dim_azimuth]}
    )
    res = xr.concat([mirror, xs], dim=dim_range)[
        {dim_range: slice(None, -1), dim_azimuth: slice(None, -1)}
    ]
    return res
