import xarray as xr
import numpy as np
from slcl1butils.legacy_ocean.ocean.spectrum_private.spectra_functions import (
    tony_omni,
    tony_spread,
    findUstar,
)
...
def set_directivity(ds, directivity, **kwargs):
    ...
    dir_alpha = kwargs.pop("dir_alpha", 10.0)
    dir_weight = kwargs.pop("dir_weight", 1.0)
    #  self.reset_derivations()
    if directivity == "alongwind":
        ds += ds.roll(phi=ds.sizes["phi"] // 2, roll_coords=False).data
        angular_weight_func = 0.5 * (
            np.tanh(
                dir_alpha
                * (np.mod(ds.phi - ds.wd + np.pi, 2 * np.pi) - np.pi + np.pi / 2.0)
            )
            - np.tanh(
                dir_alpha
                * (np.mod(ds.phi - ds.wd + np.pi, 2 * np.pi) - np.pi - np.pi / 2.0)
        ...
        ds = 2.0 * ds * (np.sin(((ds.phi - ds.wd) + np.pi) / 2) ** 2)
        ...
        raise Exception(
            "Wrong directivity for {} spectrum : {}".format(ds.name, directivity)
        )
...
def find_closest_ww3(ww3_path, lon, lat, time):
    ...
    mytime = (
        ...
        else np.datetime64(datetime(*time))
    )
        ...
        "Wave Watch III closest point @ {} km and {} minutes".format(
            np.round(
                haversine(
                    lon,
                    lat,
                    ww3spec[{"time": myind}].longitude,
                    ww3spec[{"time": myind}].latitude,
                ).data,
                2,
            ),
            (ww3spec[{"time": myind}].time - mytime).data / (1e9 * 60),
        )
    )
...
def from_ww3(
    ...
    *,
    ...
    time_index=None,
    rotate=0.0,
    shortWavesFill=False,
    clockwise_to_trigo=False,
    **kwargs
):
    """
    ...
    kmin = (2.0 * np.pi * min(specWW3.frequency).data) ** 2 / gravity
    kmax = (2.0 * np.pi * max(specWW3.frequency).data) ** 2 / gravity

    k, dk, phi, dphi = SpectrumPolarGrid(
        Nk=specWW3.sizes["frequency"],
        Nphi=specWW3.sizes["direction"],
        kmin=kmin,
        kmax=kmax,
        ksampling="log",
    )
    ...
    id = np.argsort(
        dirww3
    )  # direction vector is not monotonous. This line gives reordered direction indices over [-pi,pi[ interval
    ...
    if not time_index:
        if ("lon" in kwargs) and ("lat" in kwargs) and ("time" in kwargs):
            time_index = find_closest_ww3(
                ds, kwargs["lon"], kwargs["lat"], kwargs["time"]
            )
        else:
            time_index = 0
    ...
    if "time" in specWW3.efth.dims:
        ...
        print(
            "spectrum imported at time index {} :{} @ (lon:{}, lat:{})".format(
                time_index,
                specWW3.time[{"time": time_index}].data,
                specWW3[ww3_kwargs]["longitude"].data,
                specWW3[ww3_kwargs]["latitude"].data,
            )
        )
    if "station" in specWW3.efth.dims:
        ww3_kwargs.update({"station": station_index})
        print(
            "spectrum imported at station index {} :{}".format(
                station_index, specWW3.station[{"station": station_index}].data
            )
        )

    myspec = (
        (2.0 * np.pi)
        / np.sqrt(gravity * k[:, np.newaxis])
        * gravity
        * specWW3.efth[ww3_kwargs]
        / (8.0 * np.pi**2 * k[:, np.newaxis])
    )
    ...
    dk = (
        xr.DataArray(dk, dims="k")
        if len(np.array(dk, ndmin=1)) > 1
        else xr.DataArray(dk)
    )
    ...
    myspec.attrs.update(
        {
            "longitude": specWW3[ww3_kwargs]["longitude"].data,
            "latitude": specWW3[ww3_kwargs]["latitude"].data,
        }
    )
    ...
    try:
        myspec.attrs.update(
            {
                "wd": np.radians(specWW3[ww3_kwargs].wnddir.data + 180),
                "ws": specWW3[ww3_kwargs].wnd.data,
            }
        )  # wnddir (in WW3 files) is the direction the wind is coming from
    except:
    ...
    if clockwise_to_trigo and (rotate != 0.0):
        print(
            "Rotation angle has to be in clockwise convention because clockwise-to-trigo transformation is applied BEFORE the rotation. (Apply a minus sign on rotation angle to do it in clockwise convention)."
        )
    if clockwise_to_trigo:
        myspec = swap_clockwise_trigo(myspec)
        if "wd" in kwargs:
            kwargs.update({"wd": -kwargs["wd"]})
        if "curdir" in kwargs:
            kwargs.update({"curdir": -kwargs["curdir"]})
    if rotate != 0.0:
        myspec = apply_rotation(myspec, rotate)
        if "wd" in kwargs:
            kwargs.update({"wd": kwargs["wd"] + rotate})
        if "curdir" in kwargs:
            kwargs.update({"curdir": kwargs["curdir"] + rotate})
    if shortWavesFill:
        myspec = apply_shortWavesFill(myspec, **kwargs)

    return myspec


def apply_shortWavesFill(ds, *, kcut=4000.0, **kwargs):
    """
    ...
    if "ki" in kwargs:
        ...
        if ki > max(ds.k).data:
            raise ValueError(
                "You must respect ki < ke < kcut and ki < max k of your dataset"
            )
    else:
        ki = ds.k[-1].data
    ...
    Nk = (
        int(np.rint((np.log10(kcut) - np.log10(ds.k.min())) / dlogk)) + 1
    )  # cannot start from ki, because ki would not be on the continuing log scale of k
    ...
    if "wd" in kwargs:
        ...
        print(
            "Wind direction replaced by provided one ({} deg in local convention).".format(
                np.degrees(ds.wd)
            )
        )

    spec = from_wavenumber(
        Nk=Nk,
        Nphi=ds.sizes["phi"],
        kmin=min(ds.k).data,
        kmax=kcut,
    ...
    dims_to_drop = list(
        set(ds.coords.keys()).intersection(set(["time", "station"]))
    )  # dimensions to be dropped. Using ds.dims no dor work because singleton dimensions are not considered as dimensions.
    ...
    dk = (
        np.log(10)
        * (np.log10(spec_filled.k.max()) - np.log10(spec_filled.k.min()))
        / (Nk - 1)
        * spec_filled.k
    )
...
def from_wavenumber(**kwargs):
    ...
    ws = kwargs.pop("ws", 0.0)
    wd = np.radians(kwargs.pop("wd", 0.0))
    wd = np.arctan2(np.sin(wd), np.cos(wd))
    cur = kwargs.pop("cur", 0.0)
    curdir = np.radians(kwargs.pop("curdir", 0.0))
    ...
    if ("omega" not in kwargs) and ("fetch" not in kwargs):
        ...
        fetch = 2.2e4 * (np.arctanh((omega / 0.84) ** (-1.0 / 0.75))) ** (2.5)
    else:
        fetch = kwargs.pop("fetch")
        omega = 0.84 * np.tanh((fetch / (2.2e4)) ** 0.4) ** (-0.75)

    ds = xr.DataArray(
        np.zeros((len(k), len(phi))),
        dims=("k", "phi"),
        coords={"k": k, "phi": phi},
        attrs={
            "omega": omega,
    ...
    if name == "elfouhaily" and ws > 0.0:
        ustar = findUstar(ws, 10.0)
        ...
        magnitude = (
            curv
            / (2 * np.pi * ds.k**4)
            * (1.0 + spread * np.cos(2 * (ds.phi - ds.wd)))
        )
        ds += magnitude
    ...
    if ("sa" in kwargs) or ("sk" in kwargs) or ("sd" in kwargs):
        sa = np.array(np.double(kwargs.pop("sa", 1.0)), ndmin=1)
        sk = np.array(np.double(kwargs.pop("sk", 2 * np.pi / 10.0)), ndmin=1)
        sd = np.array(np.double(kwargs.pop("sd", 0.0)), ndmin=1)
        if len(sk) != len(sa):
        ...
        for i, (ik, it) in enumerate(zip(jk, jt)):
            ...
            swell_magnitude[ik, it] = (
                0.5 * (sa[i] / np.sqrt((ds.k[ik] * dk * ds.dphi))) ** 2
            )
            print(
                (
                    "\tk{} = {} rad/m - A{} = {} m - azi{} = {} deg - lambda{} = {} m - f{} = {} Hz"
                ).format(
                    i,
                    round(float(ds.k[ik]), 3),
                    i,
                    sa[i],
                    i,
                    round(np.rad2deg(float(ds.phi[it])), 3),
                    i,
                    round(2 * np.pi / float(ds.k[ik]), 3),
                    i,
                    round(np.sqrt(gravity * float(ds.k[ik])) / (2.0 * np.pi), 3),
                )
            )

        ds += swell_magnitude
    ds[np.where(np.isinf(ds))] = 0.0
    ds[np.where(np.isnan(ds))] = 0.0
    return ds
...
def apply_rotation(ds, angle):
    ...
    if "wd" in ds.attrs:
        ds.attrs.update(
            {"wd": angle_mpipi(ds.wd + np.radians(angle))}
        )  # wind direction has to be rotated too
    if "curdir" in ds.attrs:
        ds.attrs.update(
            {"curdir": angle_mpipi(ds.curdir + np.radians(angle))}
        )  # current direction has to be rotated too
    return ds
