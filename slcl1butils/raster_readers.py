...
def ww3_IWL1Btrack_hindcasts_30min(filename, date):
    """
    36 freq 24 directions, model output every 30min
    split into different sub-directories because of various forcing available depending of the date
    Args:
        filename:
        date:
...
def hwrf_0015_3h(fname, **kwargs):
    ...
    time_datetime = datetime.datetime.utcfromtimestamp(
        hwrf_ds.dim_0.values.astype(int) * 1e-9
    )
    hwrf_ds.attrs["time"] = time_datetime.strftime("%Y/%m/%d %H:%M:%S")

    hwrf_ds = (
        hwrf_ds.assign_coords(
            {"x": hwrf_ds.elon.values[0, :], "y": hwrf_ds.nlat.values[:, 0]}
        )
        .drop_vars(["dim_0", "elon", "nlat"])
        .rename({"u": "U10", "v": "V10"})
    )
