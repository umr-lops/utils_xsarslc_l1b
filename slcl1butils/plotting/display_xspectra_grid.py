...
from slcl1butils import spectrum_clockwise_to_trigo, spectrum_rotation
from slcl1butils.conversion_polar_cartesian import from_xCartesianSpectrum


def draw_signal_and_grid(
    slc, splitting_image, one_tiff, variable_displayed="digital_number"
):
    """
    ...
    if variable_displayed == "digital_number":
        ...
        plt.contourf(
            range_coords, azi_coords, vals.real, levels, cmap="Greys_r"
        )  # ,vmin=-1500,vmax=1500
    else:
        step = 10
        ...
        if diffatrack < 0 and diffxtrack > 0:
            sigma0cerb_pad = np.pad(
                sigma0cerb[0:diffatrack, :], ((0, 0), (0, diffxtrack))
            )
        elif diffatrack > 0 and diffxtrack < 0:
            sigma0cerb_pad = np.pad(
                sigma0cerb[:, 0:diffxtrack], ((0, diffatrack), (0, 0))
            )
        else:
            sigma0cerb_pad = np.pad(sigma0cerb, ((0, diffatrack), (0, diffxtrack)))
        sigma0cerb_pad = sigma0cerb_pad.reshape(
            (1, sigma0cerb_pad.shape[0], sigma0cerb_pad.shape[1])
        )
        logging.info(
            "sigma0cerb_pad %s mean = %s",
            sigma0cerb_pad.shape,
            np.nanmean(sigma0cerb_pad),
        )
        slc[
            "sigma0"
        ].values = sigma0cerb_pad  # on remplace par cerbere (temporairement)
        #
        sigma0 = slc["sigma0"].values.squeeze()[::step, ::step]
        ...
        roughness = sigma0
        logging.info(
            "roughness : %s nb NaN : %s", roughness.shape, np.isnan(roughness).sum()
        )
        logging.info(
            "mea roughness : %s std %s", np.nanmean(roughness), np.nanstd(roughness)
        )
        plt.contourf(
            slc["xtrack"].values[::step],
            slc["atrack"].values[::step],
            roughness,
            levels,
            cmap="Greys_r",
        )
    ...
    for lab in splitting_image:
        ...
        subset = slc["digital_number"].isel(
            azimuth=rect["azimuth"], range=rect["range"]
        )
        ...
        plt.plot(
            range_positions_corners, azimuth_positions_corners, ".-", label=lab, lw=5
        )
        mean_x = X.mean()
        mean_y = Y.mean()
...
def display_xspectra_per_domain(allspecs_per_sub_domain, part="Re"):
    ...
    for subdom in list(allspecs_per_sub_domain.keys()):  # [::-1]
        ...
        if part == "Re":
            crossSpectraRe = np.abs(
                allspecs_per_sub_domain[subdom]["cross-spectrum_2tau"]
                .mean(dim="2tau")
                .real
            )
            crossSpectraRe_red = crossSpectraRe.where(
                np.logical_and(
                    np.abs(crossSpectraRe.kx) <= 0.14, np.abs(crossSpectraRe.ky) <= 0.14
                ),
                drop=True,
            )
            crossSpectraRe_red = (
                crossSpectraRe_red.rolling(kx=3, center=True)
                .mean()
                .rolling(ky=3, center=True)
                .mean()
            )
            if vmin is None:
                vmin = 0
            ...
            crossSpectraIm_red = crossSpectraIm.where(
                np.logical_and(
                    np.abs(crossSpectraIm.kx) <= 0.14, np.abs(crossSpectraIm.ky) <= 0.14
                ),
                drop=True,
            )
            crossSpectraIm_red = (
                crossSpectraIm_red.rolling(kx=3, center=True)
                .mean()
                .rolling(ky=3, center=True)
                .mean()
            )
...
def display_xspectra_per_domain_polar(
    allspecs_per_sub_domain, heading, part="Re", min_wavelength=100
):
    """
    ...
    for subdom in list(allspecs_per_sub_domain.keys()):  # [::-1]
        ...
        logging.warning(
            "trick to match a reference spectra on cartesian coords kx and ky"
        )
        ...
        crossSpectraRe = from_xCartesianSpectrum(
            crossSpectraRe, Nphi=72, ksampling="log", **{"Nk": 60}
        )
        crossSpectraRe = spectrum_clockwise_to_trigo.apply_clockwise_to_trigo(
            crossSpectraRe
        )
        crossSpectraRe = spectrum_rotation.apply_rotation(
            crossSpectraRe, 90.0
        )  # This is for having origin at North
        ...
        crossSpectraRe_redpol = crossSpectraRe.where(
            np.abs(crossSpectraRe.k) <= 2.0 * np.pi / min_wavelength, drop=True
        )
        crossSpectraRe_redpol = (
            crossSpectraRe_redpol.rolling(k=3, center=True)
            .mean()
            .rolling(phi=3, center=True)
            .mean()
        )
        if part == "Re":
            if vmin is None:
        ...
        ax.text(
            np.radians(heading),
            (0.65 + (np.cos(np.radians(heading)) < 0) * 0.25) * max_k,
            " Azimuth",
            size=18,
            color="red",
