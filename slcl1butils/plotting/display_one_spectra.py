...
from slcl1butils import (
    conversion_polar_cartesian,
    spectrum_clockwise_to_trigo,
    spectrum_rotation,
)

cmap = mcolors.LinearSegmentedColormap.from_list(
    "", ["white", "violet", "mediumpurple", "cyan", "springgreen", "yellow", "red"]
)
try:
...
reference_oswK_1145m_60pts = np.array(
    [
        0.005235988,
        0.00557381,
        0.005933429,
...
def core_plot_part(
    ...
    ax=None,
    thecmap=None,
    limit_display_wv=100,
    title="x spec",
    tauval=2,
    azimuth_cutoffSAR=None,
    vmin=None,
    vmax=None,
    add_colorbar=True,
):
    ...
    crossSpectraRe_redpol = crossSpectraRePol.where(
        np.abs(crossSpectraRePol.k) <= 2 * np.pi / limit_display_wv, drop=True
    )
    ...
    im = crossSpectraRe_redpol_phi_new.plot(
        cmap=thecmap, alpha=0.8, vmin=vmin, vmax=vmax, add_colorbar=add_colorbar
    )
    ...
    plt.plot(
        np.radians([heading + 180, heading + 180]),
        (0.01, max(crossSpectraRe_redpol.k)),
        "r-",
    )
    plt.plot(
        np.radians([heading + 90, heading + 90]),
        (0.01, max(crossSpectraRe_redpol.k)),
        "r-",
    )
    plt.plot(
        np.radians([heading + 270, heading + 270]),
        (0.01, max(crossSpectraRe_redpol.k)),
        "r-",
    )
    ax.text(
        np.radians(heading),
        (0.65 + (np.cos(np.radians(heading)) < 0) * 0.25)
        * max(crossSpectraRe_redpol.k),
        " Azimuth",
        size=18,
        color="red",
        rotation=-heading + 90 + (np.sin(np.radians(heading)) < 0) * 180,
        ha="center",
    )
    ax.text(
        np.radians(heading + 90),
        0.80 * max(crossSpectraRe_redpol.k),
        " Range",
        size=18,
        color="red",
        rotation=-heading + (np.cos(np.radians(heading)) < 0) * 180,
        ha="center",
    )
    display_xspectra_grid.circle_plot(ax, r=[100, 200, 400])
    if azimuth_cutoffSAR:
        add_azimuth_cutoff_lines_on_polar_spec_fig.add_azimuth_cutoff_lines(
            ax=ax, tra=heading, limit_wl_plot=limit_display_wv, azc=azimuth_cutoffSAR
        )
...
def add_polar_direction_lines(deg_step=30):
    ...
    for yy, theta in enumerate(np.arange(0, 360, deg_step)):
        ...
        if abs(rotn) > 90:  # on retourne le label si ca depasse 90 ou -90
            rotn = (rotn + 180) % 180
        plt.annotate(
            "%i°" % np.rint(rotn), xy=(x3, y3), rotation=rotn, fontsize=6, color="grey"
        )
        # if yy==20:
        #     break


def display_polar_spectra(
    allspecs,
    heading,
    part="Re",
    limit_display_wv=100,
    ...
    for tauval in [tau_id]:
        if part == "Im":
            ...
            if "2tau" not in allspecs.dims:
                ...
                if allspecs[varname % tauval].dtype in [np.complex_, np.complex]:
                    coS = (
                        allspecs[varname % tauval].mean(dim="%stau" % tauval).imag
                    )  # co spectrum = imag part of cross-spectrum
                else:
            ...
            thecmap = cmap
            coS = abs(
                allspecs[varname % tauval].mean(dim="%stau" % tauval).real
            )  # co spectrum = real part of cross-spectrum
        ...
        #                                                                        reference_oswK_1145m_60pts[
        #                                                                            -1]})
        new_spec_Polar = conversion_polar_cartesian.from_xCartesianSpectrum(
            coS, Nphi=72, ksampling="log", **{"k": reference_oswK_1145m_60pts}
        )
        # new_spec_Polar = xsarsea.conversion_polar_cartesian.from_xCartesianSpectrum(coS,Nphi=72,
        #                                                                             ksampling='log',
        ...
        crossSpectraRePol = spectrum_rotation.apply_rotation(
            crossSpectraRePol, -270.0
        )  # sept 22: it seems consistent with what we see in the cart spectra, while -90 is opposite direction
        ...
        im = core_plot_part(
            ...
            ax=ax,
            thecmap=thecmap,
            limit_display_wv=limit_display_wv,
            title=title,
            tauval=tauval,
        )
        if outputfile:
...
def plot_a_single_xspec_cart_l1b_iw(
    ds,
    polarization,
    tile_line_i,
    tile_sample_i,
    ...
    if cat_xspec == "inter":
        ...
        varname = (
            "xspectra_%stau" % tau_id
        )  # it is mandatory to recombined Re+1j*Im dans the L1B
    set_xspec = ds[{"tile_line": tile_line_i, "tile_sample": tile_sample_i}].sel(
        {"pol": polarization}
    )[varname]
    if "k_az" in set_xspec.dims or skip_symmetrize is True:
        ...
        set_xspec = symmetrize_xspectrum(
            set_xspec, dim_range="k_rg", dim_azimuth="k_az"
        )  #
    ...
    _ = display_cartesian_spectra(
        ...
        rolling=True,
        title_fontsize=8,
        circles_wavelength=circles_wavelength,
...
def display_cartesian_spectra(
    ...
    rolling=False,
    dpi=100,
    circles_wavelength=[50, 100, 200, 400],
    add_colorbar=True,
    vmax=None,
):
    """
    ...
    if cat_xspec == "intra":
        ...
        if part == "Im":
            ...
            if allspecs.dtype in [np.complex_, np.complex, np.complex64]:
                coS = allspecs.mean(
                    dim="%stau" % tauval
                ).imag  # co spectrum = imag part of cross-spectrum
            else:
                coS = allspecs.mean(
                    dim="%stau" % tauval
                )  # co spectrum = imag part of cross-spectrum
            ...
            thecmap = cmap
            coS = abs(
                allspecs.mean(dim="%stau" % tauval).real
            )  # co spectrum = real part of cross-spectrum
    ...
    crossSpectraRe_red = coS.where(
        np.logical_and(
            np.abs(coS[kx_varname]) <= 0.14, np.abs(coS[ky_varname]) <= 0.14
        ),
        drop=True,
    )
    ...
    if rolling:
        crossSpectraRe_red = (
            crossSpectraRe_red.rolling({kx_varname: 3}, center=True)
            .mean()
            .rolling({ky_varname: 3}, center=True)
            .mean()
        )
    ...
    plt.plot(
        [0, 0],
        [-2.0 * np.pi / limit_display_wv, 2.0 * np.pi / limit_display_wv],
        "c--",
        alpha=0.7,
    )
    ...
    im = crossSpectraRe_red.plot(
        cmap=thecmap, alpha=0.8, add_colorbar=add_colorbar, vmax=vmax
    )
