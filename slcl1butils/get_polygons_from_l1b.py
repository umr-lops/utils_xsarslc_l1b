...
def get_swath_tiles_polygons_from_l1bgroup(l1b_ds,polarization, swath_only=False, ik=0, burst_type='intra', **kwargs):
    """
    ...
    for uu in gege:
        ...
        if burst_type == 'intra':
            if 'burst_corner_longitude' in l1b_ds:
                burst_lon_corners = l1b_ds['burst_corner_longitude'].sel(tile_line=itile_line).values.flatten().tolist()
                burst_lat_corners = l1b_ds['burst_corner_latitude'].sel(tile_line=itile_line).values.flatten().tolist()
