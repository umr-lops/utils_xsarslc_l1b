...
def coloc_tiles_from_l1bgroup_with_raster(l1b_ds, raster_bb_ds, apply_merging=True, method=1):
    ...
    if (method==0):
        ...
        for cpt,poly_tile in enumerate(poly_tiles):
            ...
            #
            _anc_ds = xr.Dataset(data_vars={'U10':U10,'V10':V10},coords={'burst':coordinates['ibursts'][cpt],'tile_sample':coordinates['itile_samples'][cpt],'tile_line':coordinates['itile_lines'][cpt]}).expand_dims(['burst','tile_sample','tile_line'])
...
def do_coloc_L1B_with_raster_SAFE(full_safe_file,ancillary_list,skip=True):
    ...
    for _file in files : 
        ...
        if (sth>0):
            ...
            if ( (skip==True) & os.path.isfile(pathout+fileout) ):
